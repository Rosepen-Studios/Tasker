...
## - `0 (Square)`.
## - `1 (Circle)`.
@export_enum("Square", "Circle") var type: int = 1
...
func _ready():
	position = Vector2(1152-160,648-90)
	set_process(false)
	emitting = not Engine.is_editor_hint()
...
func _get_random_amount():
	return round(randi_range(amount / 2.0, amount * 2.0))


func _get_random_position():
	var x = randi_range(0, visibility_rect.size.x)
	var y = randi_range(0, visibility_rect.size.y)
